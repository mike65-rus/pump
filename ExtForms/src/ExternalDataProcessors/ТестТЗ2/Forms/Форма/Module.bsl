...
&НаСервереБезКонтекста
Процедура ЗагрузитьSK007Справочник()
	пПапкаНСИ="D:\educ\STORAGE";
	пИмяСправочника="SK007";
...
КонецПроцедуры
&НаСервереБезКонтекста
Процедура ЗагрузитьM001Справочник()
	пПапкаНСИ="D:\educ\STORAGE";
	пИмяСправочника="M001";
	пТаб=Символы.Таб;
	ИмяСтартовогоАтрибута="MKBLIST";
	ИмяКласса="KLASS";
	ИмяБлока="BLOCK";
	пФайл=пПапкаНСИ+ПолучитьРазделительПутиСервера()+пИмяСправочника+".xml";
	...
	Пока Парсер.Прочитать() Цикл
		 Если Парсер.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
		 	...
		 	КонецЕсли;	
	        ПолныйПуть = ПостроитьПолныйПуть(СтекИмен);
	        Если ПолныйПуть="/"+ИмяСтартовогоАтрибута+"/"+ИмяКласса+"/item" Тогда
	        	пНаименование=Парсер.ЗначениеАтрибута("NAME_KLASS");
	        	Если пНаименование=Неопределено  Тогда
	        		Продолжить;
	        	КонецЕсли;	
				РазименовывателиПИ = Документ.СоздатьРазыменовательПИ(Документ);	       
 				Запрос=ПолныйПуть+"[@NAME_KLASS='"+пНаименование+"']/BLOCK";
	        	Запрос="count("+Запрос+")";
	        	Путь=Документ.СоздатьВыражениеXPath(Запрос,РазименовывателиПИ);
				Результат = Путь.Вычислить(Документ);
				Если Результат.ЧисловоеЗначение=0 Тогда
					Продолжить;
				КонецЕсли;		
				пКод=Прав(пНаименование,9);
				пНаименование=СтрЗаменить(пНаименование,пКод,"");
				Если СтрНайти(пКод,"A09")>0 Тогда
					пКод=СтрЗаменить(пКод,"A09","B99");
				КонецЕсли;
				пКод=СтрЗаменить(пКод,"(","");
				пКод=СтрЗаменить(пКод,")","");
				
//				Сообщить(пКод+пТаб+пНаименование);
				пСпрСсылка=пСпрМенеджер.НайтиПоКоду(пКод);
				пНовый=пСпрСсылка.Пустая();
				Если НЕ пНовый Тогда
					пГруппа=пСпрСсылка.ПолучитьОбъект();
					пКодГруппы1=пГруппа.Код;
				Иначе
					пГруппа=пСпрМенеджер.СоздатьГруппу();
					пГруппа.Код=пКод;
					пГруппа.Наименование=пНаименование;
					пГруппа.Записать();
					пКодГруппы1=пКод;
				КонецЕсли;	
			ИначеЕсли ПолныйПуть="/"+ИмяСтартовогоАтрибута+"/"+ИмяКласса+"/item/"+ИмяБлока+"/item1" Тогда	        	
	        	пНаименование=Парсер.ЗначениеАтрибута("NAME_BLOCK");
	        	Если пНаименование=Неопределено  Тогда
	        		Продолжить;
	        	КонецЕсли;	
				РазименовывателиПИ = Документ.СоздатьРазыменовательПИ(Документ);	       
 				Запрос=ПолныйПуть+"[@NAME_BLOCK='"+пНаименование+"']/RUBR/item2[string-length(@ID)=3]/@ID";
	        	Путь=Документ.СоздатьВыражениеXPath(Запрос,РазименовывателиПИ);
				Результат = Путь.Вычислить(Документ);
				пСпис=Новый СписокЗначений;
				...
				Пока Эл<>Неопределено Цикл
					...
					Эл=Результат.ПолучитьСледующий();
				КонецЦикла;
				Если пСпис.Количество()=0 Тогда
					Продолжить;
				КонецЕсли;
				пСпис.СортироватьПоЗначению(НаправлениеСортировки.Возр);
				пКод=Строка(пСпис.Получить(0).Значение);
				пКод=пКод+"-"+Строка(пСпис.Получить(пСпис.Количество()-1).Значение);
//				Сообщить(пКод+пТаб+пНаименование);
				пСпрСсылка=пСпрМенеджер.НайтиПоКоду(пКод);
				пНовый=пСпрСсылка.Пустая();
				Если НЕ пНовый Тогда
					пГруппа=пСпрСсылка.ПолучитьОбъект();
					пКодГруппы2=пГруппа.Код;
				Иначе
					пГруппа=пСпрМенеджер.СоздатьГруппу();
					пРодительСсылка=пСпрМенеджер.НайтиПоКоду(пКодГруппы1);
					пГруппа.Родитель=пРодительСсылка;
					пГруппа.Код=пКод;
					пГруппа.Наименование=пНаименование;
					пГруппа.Записать();
					пКодГруппы2=пКод;
			...
			ИначеЕсли ПолныйПуть="/"+ИмяСтартовогоАтрибута+"/"+ИмяКласса+"/item/"+ИмяБлока+"/item1/RUBR/item2" Тогда	        	
	        	пКод=Парсер.ЗначениеАтрибута("ID");
	        	пНаименование=Парсер.ЗначениеАтрибута("NAME");
	        	пКодРодителя=Парсер.ЗначениеАтрибута("ID_IERARH");
	        	пДатаНач=Парсер.ЗначениеАтрибута("DATE_BEG");
	        	пДатаОконч=Парсер.ЗначениеАтрибута("DATE_END");
	        	пДатаНачала='0001.01.01';
	        	пДатаОкончания='0001.01.01';
	        	Если пДатаНач<>Неопределено Тогда
	        		пДатаНачала=ЗначениеXmlВЗначение(пДатаНач,Тип("Дата"));
	        	КонецЕсли;	
	        	Если пДатаОконч<>Неопределено Тогда
	        		пДатаОкончания=ЗначениеXmlВЗначение(пДатаОконч,Тип("Дата"));
	        	КонецЕсли;	
	        	Если (пНаименование=Неопределено) ИЛИ (пКод=Неопределено)  Тогда
	        		Продолжить;
	        	КонецЕсли;	
	        	пЭтоГруппа=Ложь;
	        	Если СтрДлина(пКод)=3 Тогда
					РазименовывателиПИ = Документ.СоздатьРазыменовательПИ(Документ);
	 				Запрос=ПолныйПуть+"[@ID_IERARH='"+СОКРЛП(пКод)+"']";
		        	Запрос="count("+Запрос+")";
		        	Путь=Документ.СоздатьВыражениеXPath(Запрос,РазименовывателиПИ);
					Результат = Путь.Вычислить(Документ);
					пЭтоГруппа=(Результат.ЧисловоеЗначение>1);
					Если НЕ пЭтоГруппа Тогда
						пПоследнийРодитель=пКодГруппы2;
					Иначе
						пПоследнийРодитель=пКодРодителя;
					КонецЕсли;		
				КонецЕсли;	
				пМассив=СтрРазделить(пНаименование,"-");
				Для ш= 1 ПО пМассив.Количество()-1 Цикл
					Если ш=1 Тогда
						пНаименование="";
					КонецЕсли;	
					пНаименование=пНаименование+?(ш=1,"","-")+пМассив[ш];
				КонецЦикла;	
				пКод=СОКРЛП(пКод);
				пНаименование=СОКРЛП(пНаименование);
//				Сообщить(пКод+пТаб+пНаименование+?(пЭтоГруппа," <Группа>"," <Элемент>"));
				пСпрСсылка=пСпрМенеджер.НайтиПоКоду(пКод);
				пНовый=пСпрСсылка.Пустая();
				Если пЭтоГруппа Тогда
					Если НЕ пНовый Тогда
						пГруппа=пСпрСсылка.ПолучитьОбъект();
						//@skip-warning
						пКодГруппы3=пГруппа.Код;
					Иначе
						пГруппа=пСпрМенеджер.СоздатьГруппу();
						пРодительСсылка=пСпрМенеджер.НайтиПоКоду(пПоследнийРодитель);
						пГруппа.Родитель=пРодительСсылка;
						пГруппа.Код=пКод;
						пГруппа.Наименование=пНаименование;
						пГруппа.Записать();
				...
				Иначе
					Если НЕ пНовый Тогда
						...
						пГруппа.Наименование=пНаименование;
						Попытка
							пГруппа.DATE_BEG=пДатаНачала;
						Исключение
						КонецПопытки;	
						Попытка
							пГруппа.DATE_END=пДатаОкончания;
						Исключение
						КонецПопытки;	
						пГруппа.Записать();
					Иначе
						пГруппа=пСпрМенеджер.СоздатьЭлемент();
						пРодительСсылка=пСпрМенеджер.НайтиПоКоду(пПоследнийРодитель);
						пГруппа.Родитель=пРодительСсылка;
						пГруппа.Код=пКод;
						пГруппа.Наименование=пНаименование;
...
КонецПроцедуры
&НаСервереБезКонтекста
Процедура ОбработкаСправочникаM001()
	пПапкаНСИ="D:\educ\STORAGE";
	пИмяСправочника="M001";
	пСпрМенеджер=Справочники["M001"];
	...
	|	M001.Наименование,
	|	M001.ЭтоГруппа,
	|	M001.Родитель.Код КАК КодРодителя,
	|	M001.Родитель.Ссылка КАК РодительССылка
	|ИЗ
	|	Справочник.M001 КАК M001
	|ГДЕ
	|	M001.ЭтоГруппа = ЛОЖЬ
	|УПОРЯДОЧИТЬ ПО
	|	Код";
	пТЗ=Запрос.Выполнить().Выгрузить();
	Для Каждого пСтр из пТЗ Цикл
		пКод=СОКРЛП(пСтр.Код);
		пКодРодителя=СОКРЛП(пСтр.КодРодителя);
		Если СтрДлина(пКод)<=3 Тогда
			Продолжить;
		КонецЕсли;
		Если (Лев(пКод,3)<>Лев(пКодРодителя,3)) Тогда
			пСпрСсылка=Справочники.M001.НайтиПоКоду(Лев(пКод,3));
			Если пСпрСсылка.Пустая() Тогда
				Продолжить;
			КонецЕсли;
			Если НЕ пСпрСсылка.ЭтоГруппа Тогда
				пНоваяГруппа=Справочники.M001.СоздатьГруппу();
				пНоваяГруппа.Родитель=пСпрСсылка.Родитель;
				пНоваяГруппа.Наименование=пСпрСсылка.Наименование;
				пНоваяГруппа.Код=пСпрСсылка.Код;
				пСтарыйОбъект=пСпрСсылка.ПолучитьОбъект();
				пСтарыйОбъект.Удалить();
				пНоваяГруппа.Записать();
				пСпрСсылка=пНоваяГруппа.Ссылка;
			КонецЕсли;	
			пСпрСсылка1=Справочники.M001.НайтиПоКоду(пКод);
			пСпрОбъект1=пСпрСсылка1.ПолучитьОбъект();
			пСпрОбъект1.Родитель=пСпрСсылка;
			пСпрОбъект1.Записать();
...
&НаСервереБезКонтекста
Процедура ЗагрузитьF003_OСправочник()
	пПапкаНСИ="D:\educ\STORAGE";
	пИмяСправочника="F003_O";

//	пНашКодЛПУ=СОКРЛП(Справочники.НастройкиПУМП.КодЛПУ.Значение);
	ИмяПоляИД="NUMBER";	
	ИмяПоляДатаНач="DATE_BEG";	
	ИмяПоляНаименование="NUMBER";
	ИмяСтартовогоАтрибута="MO_LIST";
	пФайл=пПапкаНСИ+ПолучитьРазделительПутиСервера()+пИмяСправочника+".xml";
	пСпрМенеджер=Справочники[пИмяСправочника];
	пСтруктураСправочника=Новый Структура;
	Для Каждого Реквизит Из Метаданные.Справочники[пИмяСправочника].Реквизиты Цикл
	...
	Пока Парсер.Прочитать() Цикл
		 Если Парсер.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
	        ...
	        ПолныйПуть = ПостроитьПолныйПуть(СтекИмен);
            Если ПолныйПуть = "/"+ИмяСтартовогоАтрибута+"/item" Тогда
            	MCOD=Парсер.ПолучитьАтрибут("MCOD");
            ИначеЕсли ПолныйПуть= "/"+ИмяСтартовогоАтрибута+"/item/PODR" Тогда
    	        	ID_PODR=Парсер.ПолучитьАтрибут("ID_PODR");
            ИначеЕсли ПолныйПуть="/"+ИмяСтартовогоАтрибута+"/item/PODR/PARTLIST/item1" Тогда
            	ID_PART=Парсер.ПолучитьАтрибут("ID_PART");
            	NAME=Парсер.ПолучитьАтрибут("NAME");
            	USL_OK=Парсер.ПолучитьАтрибут("USL_OK");
            	PROF=Парсер.ПолучитьАтрибут("PROF");
            	DATE_BEG=Парсер.ПолучитьАтрибут("DATE_BEG");
            	DATE_END=Парсер.ПолучитьАтрибут("DATE_END");
            КонецЕсли;
		 ИначеЕсли Парсер.ТипУзла = ТипУзлаXML.КонецЭлемента Тогда
			 Если  ПолныйПуть="/"+ИмяСтартовогоАтрибута+"/item/PODR/PARTLIST/item1" Тогда
			 	пСпрСсылка=пСпрМенеджер.НайтиПоКоду(ID_PART);
			 	Если пСпрСсылка.Пустая() Тогда
			 		пСпрОбъект=пСпрМенеджер.СоздатьЭлемент();
			 	Иначе
			 		пСпрОбъект=пСпрСсылка.ПолучитьОбъект();	
			 	КонецЕсли;	
			 	пСпрОбъект.Код=ID_PART;
			 	пСпрОбъект.Наименование=NAME;
			 	пСпрОбъект.MCOD=MCOD;
			 	пСпрОбъект.ID_PODR=ЗначениеXmlВЗначение(ID_PODR,Тип("Число"));;
			 	пСпрОбъект.USL_OK=ЗначениеXmlВЗначение(USL_OK,Тип("Число"));
			 	пСпрОбъект.PROF=ЗначениеXmlВЗначение(PROF,Тип("Число"));
			 	пСпрОбъект.DATE_BEG=ЗначениеXmlВЗначение(DATE_BEG,Тип("Дата"));
			 	пСпрОбъект.DATE_END=ЗначениеXmlВЗначение(DATE_END,Тип("Дата"));
				пСпрОбъект.Записать();			 	
			 КонецЕсли;			 	
	         СтекИмен.Удалить(СтекИмен.Количество() - 1);
             ПолныйПуть = ПостроитьПолныйПуть(СтекИмен);
...
//	ЗагрузитьSK007Справочник();	
	// ЗагрузитьM001Справочник();
//	ОбработкаСправочникаM001();
	ЗагрузитьF003_OСправочник();	
КонецПроцедуры
