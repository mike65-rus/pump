...
Процедура ПрикрепитьФайл(пИмяФайла,пСсылкаНаВладельца)
		ФайлДокумента = Новый Файл(пИмяФайла);
		...
		ПараметрыДобавленияФайла=РаботаСФайлами.ПараметрыДобавленияФайла(Неопределено);
		ПараметрыДобавленияФайла.ВладелецФайлов=пСсылкаНаВладельца;
		ПараметрыДобавленияФайла.ИмяБезРасширения=ФайлДокумента.ИмяБезРасширения;
		ПараметрыДобавленияФайла.РасширениеБезТочки=СтрЗаменить(ФайлДокумента.Расширение,".","");
		НоваяСсылкаНаФайл=РаботаСФайлами.НоваяСсылкаНаФайл(пСсылкаНаВладельца);
		//@skip-warning
		Результат = РаботаСФайлами.ДобавитьФайл(
		ПараметрыДобавленияФайла, АдресВременногоХранилищаФайла, 
			АдресВременногоХранилищаТекста, "Добавлен при импорте справочника", 
			НоваяСсылкаНаФайл);
КонецПроцедуры
			
Процедура ВыполнитьЗагрузкуИзФайла(Данные) Экспорт
	Перем ИмяХмлФайла,МассивУдаляемыхФайлов;
    ...
    Для Каждого Элемент Из Архив.Элементы Цикл
        ...
        МассивИзвлеченныхФайлов.Добавить(Элемент.Имя);
	...
	КонецЕсли;
	// Прикрепление распакованных файлов к эаписи справочника всех МЗРФфайлов
	пСправочник=Справочники.СправочникиМЗРФ;
	пСсылка=пСправочник.НайтиПоКоду("МЗРФ_МКБ");
	пМассивПрикрепленныхФайлов=Новый Массив;
	РаботаСФайлами.ЗаполнитьПрисоединенныеФайлыКОбъекту(пСсылка,пМассивПрикрепленныхФайлов);
	Для Каждого пФайлИзЗип Из МассивИзвлеченныхФайлов Цикл
		пУжеПрикреплен=Ложь; 
		Для Каждого пФайл Из пМассивПрикрепленныхФайлов Цикл
			пФайлОбъект=пФайл.ПолучитьОбъект();
			Если НРЕГ(пФайлИзЗип)=НРЕГ(Строка(пФайлОбъект)+"."+пФайлОбъект.Расширение) Тогда
				Если НЕ пФайлОбъект.ПометкаУдаления Тогда
					пУжеПрикреплен=Истина;
				Прервать;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		Если Не пУжеПрикреплен Тогда
			ПрикрепитьФайл(КаталогВременныхФайлов()+пФайлИзЗип,пСсылка);
		КонецЕсли;	
	КонецЦикла;
	// конец прикрепления
